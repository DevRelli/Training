...
def get_status(url):
    ...
    with REPORT_LOCK: # Irá bloquear as threads para que todas possam executar o processamento a seguir em fila para não ocorrer nenhuma convergência na estrutura de dados
...
with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
    futures = [] # Resultado de uma thread, como se fosse uma promessa de resultado da thread
    for url in urls:
    ...
    for future in concurrent.futures.as_completed(futures):
        try:
            ...
            print(f"Status code for {url} id {code}")
